# go-ruby-nats documentation

**Pure-Go (CGO=0), MRI-faithful reimplementation of the Ruby nats client — the NATS messaging client, speaking the NATS protocol over its own socket.**

`go-ruby-nats/nats` is a faithful, pure-Go (zero cgo) reimplementation of Ruby's `nats`,
matching reference Ruby (MRI) behaviour. The module path is
`github.com/go-ruby-nats/nats`.
...
!!! success "Status: pure-Go, CGO=0, differential-tested"
    A faithful pure-Go port of Ruby's `nats`, validated against reference Ruby, at 100%
...
```sh
go get github.com/go-ruby-nats/nats
```
...
| Repo | What it is |
| --- | --- |
| [`nats`](https://github.com/go-ruby-nats/nats) | the library — Ruby's `nats` in pure Go |
| [`docs`](https://github.com/go-ruby-nats/docs) | this documentation site (MkDocs Material, versioned with mike) |
| [`go-ruby-nats.github.io`](https://github.com/go-ruby-nats/go-ruby-nats.github.io) | the organization landing page (Hugo) |
| [`brand`](https://github.com/go-ruby-nats/brand) | logo and brand assets |
...
Source lives at [github.com/go-ruby-nats/nats](https://github.com/go-ruby-nats/nats).
