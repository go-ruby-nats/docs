...
`go-ruby-nats/nats` reimplements Ruby's `nats` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-nats/nats` directly, with no Ruby runtime;
